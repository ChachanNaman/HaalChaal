...
5. **Nothing is deployed yet.** Backend only runs locally behind an ngrok tunnel; dashboard has only been run locally (`npm run dev` / `npm run start`). Vercel (dashboard) + Render or Fly (backend) are the planned targets per the PRD, not done.
